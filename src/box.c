...
//Ryolo
Rbox float_to_Rbox_stride(float* f, int stride)
{
    Rbox b = { 0 };
    b.x = f[0];
    b.y = f[1 * stride];
    b.w = f[2 * stride];
    b.h = f[3 * stride];
    b.r1 = f[4 * stride];//Ryolo
    b.r2 = f[5 * stride];//Ryolo
    b.jud = f[6 * stride];//Ryolo
    b.adv = f[7 * stride];//Ryolo
    return b;
}
...
//Ryolo
float Rbox_iou_kind(Rbox a, Rbox b, IOU_LOSS iou_kind)
{
    //IOU, GIOU, MSE, DIOU, CIOU
    switch (iou_kind) {
    case IOU: return Rbox_iou(a, b);
    case GIOU: return Rbox_giou(a, b);
    case DIOU: return Rbox_diou(a, b);
    case CIOU: return Rbox_ciou(a, b);
    }
    return Rbox_iou(a, b);
}
//Ryolo
...
//Ryolo
dxrep dx_Rbox_iou(Rbox pred, Rbox truth, IOU_LOSS iou_loss) {
    boxabs pred_tblr = to_Rtblr(pred);
    float pred_t = fmin(pred_tblr.top, pred_tblr.bot);
    float pred_b = fmax(pred_tblr.top, pred_tblr.bot);
    float pred_l = fmin(pred_tblr.left, pred_tblr.right);
    float pred_r = fmax(pred_tblr.left, pred_tblr.right);
    //dbox dover = derivative(pred,truth);
    //dbox diouu = diou(pred, truth);
    boxabs truth_tblr = to_Rtblr(truth);
#ifdef DEBUG_PRINTS
    printf("\niou: %f, giou: %f\n", Rbox_iou(pred, truth), Rbox_giou(pred, truth));
    printf("pred: x,y,w,h: (%f, %f, %f, %f) -> t,b,l,r: (%f, %f, %f, %f)\n", pred.x, pred.y, pred.w, pred.h, pred_tblr.top, pred_tblr.bot, pred_tblr.left, pred_tblr.right);
    printf("truth: x,y,w,h: (%f, %f, %f, %f) -> t,b,l,r: (%f, %f, %f, %f)\n", truth.x, truth.y, truth.w, truth.h, truth_tblr.top, truth_tblr.bot, truth_tblr.left, truth_tblr.right);
#endif
    //printf("pred (t,b,l,r): (%f, %f, %f, %f)\n", pred_t, pred_b, pred_l, pred_r);
