...
#include "Ryolo_layer.h"//Ryolo
#include "logistic_layer.h"//Syolo
...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[ryolo]") == 0) return RYOLO;//Ryolo
    if (strcmp(type, "[logistic]") == 0) return LOGXENT;//Syolo
...
//Ryolo
layer parse_Ryolo(list* options, size_params params)
{
    int classes = option_find_int(options, "classes", 20);
    int total = option_find_int(options, "num", 1);
    int num = total;
    ...
    fprintf(stderr, "[Ryolo] params: iou loss: %s (%d), iou_norm: %2.2f, obj_norm: %2.2f, cls_norm: %2.2f, delta_norm: %2.2f, scale_x_y: %2.2f\n",
        iou_loss, l.iou_loss, l.iou_normalizer, l.obj_normalizer, l.cls_normalizer, l.delta_normalizer, l.scale_x_y);
