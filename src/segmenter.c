...
#include "image.h"
char* GetFilename_seg(char* p)//Syolo
{
	static char name[20] = { "" };
	char* q = strrchr(p, '\\') + 1;
	strncpy(name, q, 23);
...
void train_segmenter(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int display)
{
   ...
   // network* nets = (network*)xcalloc(ngpus, sizeof(network));
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        printf("%ld, %.3f: %f, %f avg, %.9f rate, %lf seconds, %ld images\n", get_current_batch(net), (float)(*net.seen)/N, loss, avg_loss, get_current_rate(net), what_time_is_it_now()-time, *net.seen);
        free_data(train);
        //if(*net.seen/N > epoch){
        //    epoch = *net.seen/N;
        //    char buff[256];
...
void predict_segmenter(char *datafile, char *cfg, char *weights, char *filename)
{
    ...
    while(1){
        if(filename){
            ...
            for (int i = 0; i < m; ++i) {
                char* path = paths[i];
                image im = load_image(path, net.w, net.h, net.c);
				...
				//save_image_bmp(prmask, "prmask");
				char b[2048];
				sprintf(b, "./result/%s", GetFilename_seg(path));
				save_image_bmp(pred, b);
				printf("save %s successfully!\n", GetFilename_seg(path));
				free_image(im);
				free_image(sized);
                //free_image(pred);
				//free_image(prmask);
