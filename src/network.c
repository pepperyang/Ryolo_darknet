...
int resize_network(network *net, int w, int h)
{
    ...
    for (i = 0; i < net->n; ++i){
        ...
        else if (l.type == RYOLO) {//Ryolo
            resize_Ryolo_layer(&l, w, h);
        }
...
int num_detections(network *net, float thresh)
{
    ...
    for (i = 0; i < net->n; ++i) {
        ...
        //Ryolo
        if (l.type == RYOLO) {
            s += Ryolo_num_detections(l, thresh);
        }
...
void fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter)
{
    ...
    for (j = 0; j < net->n; ++j) {
        ...
        //Ryolo
        if (l.type == RYOLO) {
            int count = get_Ryolo_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
            dets += count;
            if (prev_classes < 0) prev_classes = l.classes;
            else if (prev_classes != l.classes) {
                printf(" Error: Different [Ryolo] layers have different number of classes = %d and %d - check your cfg-file! \n",
                    prev_classes, l.classes);
            }
        }
        if (l.type == GAUSSIAN_YOLO) {
