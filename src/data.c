...
Rbox_label* read_Rboxes(char* filename, int* n, int net_w, int net_h)
{
    ...
    const int img_hash = (custom_hash(filename) % max_obj_img) * max_obj_img;
    //printf(" img_hash = %d, filename = %s; ", img_hash, filename);
    float x, y, h, w, r1, r2, ra, rb;//Ryolo
    float theta = 0.0;
    
    float pi = 3.14159265359;//Ryolo
    int jud = 0;//Ryolo
    int adv = 0;//Ryolo
    int id;
    int count = 0;
    ...
    while (fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &theta, &ra, &rb) == 6) {//Ryolo
        ...
        if (0 < theta && theta < pi ) { jud = 1; }
        if (-pi <  theta && theta < 0) { jud = 0; }

		if (0 < theta && theta < 0.5*pi || -0.5*pi <  theta && theta < 0) {adv = 1; }
		if (0.5*pi < theta && theta < pi || -pi < theta && theta < -0.5*pi){adv = 0; }
        //对于特殊情况
        if (theta == 0 ) { jud = 1; adv = 1; }
        if (theta == 0.5*pi) { jud = 1; adv = 0; }
        if (theta == pi) { jud = 0; adv = 0; }
        if (theta == -0.5*pi) { jud = 0; adv = 1; }

        if (theta == 0. || theta == pi || theta == - pi) { w = ra; h = rb; r1 = 1; r2 = 0;}
        if (theta == 0.5*pi || theta == -0.5*pi) { w = rb; h = ra; r1 = 0; r2 = 1; }
        //判断四个顶点位置
        ...
        Rboxes[count].track_id = count + img_hash;
        //printf(" Rboxes[count].track_id = %d, count = %d \n", Rboxes[count].track_id, count);
        Rboxes[count].id = id;
        Rboxes[count].x = x;
        Rboxes[count].y = y;
        Rboxes[count].h = h;
        Rboxes[count].w = w;
        Rboxes[count].r1 = r1;//Ryolo
        Rboxes[count].r2 = r2;//Ryolo
        Rboxes[count].jud = jud;//Ryolo
        Rboxes[count].adv = adv;//Ryolo
        Rboxes[count].left = x - w / 2;
        Rboxes[count].right = x + w / 2;
        Rboxes[count].top = y - h / 2;
        Rboxes[count].bottom = y + h / 2;
        ++count;
    }
    fclose(file);
    *n = count;
...
//Ryolo
void correct_Rboxes(Rbox_label* Rboxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    int i;
    for (i = 0; i < n; ++i) {
        if (Rboxes[i].x == 0 && Rboxes[i].y == 0) {
            Rboxes[i].x = 999999;
            Rboxes[i].y = 999999;
            Rboxes[i].w = 999999;
            Rboxes[i].h = 999999;
        ...
        if ((Rboxes[i].x + Rboxes[i].w / 2) < 0 || (Rboxes[i].y + Rboxes[i].h / 2) < 0 ||
            (Rboxes[i].x - Rboxes[i].w / 2) > 1 || (Rboxes[i].y - Rboxes[i].h / 2) > 1)
        {
            Rboxes[i].x = 999999;
            Rboxes[i].y = 999999;
            Rboxes[i].w = 999999;
        ...
        Rboxes[i].left = Rboxes[i].left * sx - dx;
        Rboxes[i].right = Rboxes[i].right * sx - dx;
        Rboxes[i].top = Rboxes[i].top * sy - dy;
        Rboxes[i].bottom = Rboxes[i].bottom * sy - dy;

        if (flip) {
            float swap = Rboxes[i].left;
            Rboxes[i].left = 1. - Rboxes[i].right;
            Rboxes[i].right = 1. - swap;
        }

        Rboxes[i].left = constrain(0, 1, Rboxes[i].left);
        Rboxes[i].right = constrain(0, 1, Rboxes[i].right);
        Rboxes[i].top = constrain(0, 1, Rboxes[i].top);
        Rboxes[i].bottom = constrain(0, 1, Rboxes[i].bottom);

        Rboxes[i].x = (Rboxes[i].left + Rboxes[i].right) / 2;
        Rboxes[i].y = (Rboxes[i].top + Rboxes[i].bottom) / 2;
        Rboxes[i].w = (Rboxes[i].right - Rboxes[i].left);
        Rboxes[i].h = (Rboxes[i].bottom - Rboxes[i].top);

        Rboxes[i].w = constrain(0, 1, Rboxes[i].w);
        Rboxes[i].h = constrain(0, 1, Rboxes[i].h);
    }
}
...
//Ryolo
int fill_truth_Rdetection(const char* path, int num_boxes, int truth_size, float* truth, int classes, int flip, float dx, float dy, float sx, float sy,
    int net_w, int net_h)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    ...
    for (i = 0; i < count; ++i) {
        ...
        w = Rboxes[i].w;
        h = Rboxes[i].h;
        r1 = Rboxes[i].r1;//Ryolo
        r2 = Rboxes[i].r2;//Ryolo
        jud = Rboxes[i].jud; //Ryolo
        adv = Rboxes[i].adv; //Ryolo
        id = Rboxes[i].id;
        int track_id = Rboxes[i].track_id;
        ...
        truth[(i - sub) * truth_size + 4] = r1;//Ryolo
        truth[(i - sub) * truth_size + 5] = r2;//Ryolo
        truth[(i - sub) * truth_size + 6] = jud;//Ryolo
        truth[(i - sub) * truth_size + 7] = adv;//Ryolo
        truth[(i - sub) * truth_size + 8] = id;
        truth[(i - sub) * truth_size + 9] = track_id;
        //float val = track_id;
...
image load_image_gray(char* path, int w, int h) {
    ...
    //return load_image_stb_mask(labelpath, 1);
}
...
//Ryolo
data load_data_Rdetection(int n, char** paths, int m, int w, int h, int c, int boxes, int truth_size, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int show_imgs)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            int min_w_h = fill_truth_Rdetection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
            //for (int z = 0; z < boxes; ++z) if(truth[z*truth_size] > 0) printf(" track_id = %f \n", truth[z*truth_size + 5]);
            //printf(" truth_size = %d \n", truth_size);
            ...
            if (show_imgs && i_mixup == use_mixup)   // delete i_mixup
            {
                ...
                int t;
                for (t = 0; t < boxes; ++t) {
                    Rbox b = float_to_Rbox_stride(d.y.vals[i] + t * truth_size, 1);
                    if (!b.x) break;
                    int left = (b.x - b.w / 2.) * ai.w;
                    int right = (b.x + b.w / 2.) * ai.w;
                    int top = (b.y - b.h / 2.) * ai.h;
...
//Ryolo
data load_data_Rdetection(int n, char** paths, int m, int w, int h, int c, int boxes, int truth_size, int classes, int use_flip, int gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int show_imgs)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    ...
    for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            fill_truth_Rdetection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
...
void *load_thread(void *ptr)
{
    ...
    }else if (a.type == RDETECTION_DATA) {//Ryolo
        *a.d = load_data_Rdetection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
            a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive, a.contrastive_jit_flip, a.show_imgs);
    }
...
data load_data_seg(int n, char** paths, int m, int w, int h, int classes, int min, int max, float angle, float aspect, float hue, float saturation, float exposure, int div)
{
    ...
    d.y.vals = xcalloc(d.X.rows, sizeof(float*));

    for (i = 0; i < n; ++i) {
       // image orig = load_image_color(random_paths[i], 0, 0);
        image orig = load_image(random_paths[i], w, h,1);
        //augment_args a;
        //a.rad = 0;
        //a.scale = 1;
        //a.w = orig.w;
       ...
       //if (flip) flip_image(mask);
            //---------------------随机裁剪缩放-------------------------
        d.y.vals[i] = mask.data;
            //---------------------随机裁剪缩放-------------------------

        //show_image(sized_m, "part", 0);
        //show_image(mask, "mask", 0);
        //show_image(sized,"size",0);
        //save_image_bmp(orig, "sized");
        //save_image_bmp(mask, "sized_m");
