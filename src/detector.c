...
//Ryolo
void train_Rdetector(char* datacfg, char* cfgfile, char* weightfile, int* gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    list* options = read_data_cfg(datacfg);
    char* train_images = option_find_str(options, "train", "data/train.txt");
    char* valid_images = option_find_str(options, "valid", train_images);
   ...
   data train, buffer;
    ...
    for (k = 0; k < net.n; ++k) {
        layer lk = net.layers[k];
        if (lk.type == RYOLO || lk.type == GAUSSIAN_YOLO || lk.type == REGION) {
            l = lk;
            printf(" Detection layer: %d - type = %d \n", k, l.type);
        }
    }
...
//Ryolo
void test_Rdetector(char* datacfg, char* cfgfile, char* weightfile, char* filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char* outfile, int letter_box, int benchmark_layers)
{
    list* options = read_data_cfg(datacfg);
    char* name_list = option_find_str(options, "names", "data/names.list");
    ...
    while (1) {
        if (filename) {
            ...
            for (int i = 0; i < m; ++i) {
                ...
                if (outfile) {
                    ...
                    char b[2048];
                    sprintf(b, "./result/%s", GetFilename(path));
        ...
        //    else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind, l.beta_nms);
        //}
        //draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, NULL);
        //save_image(im, "predictions");
        //if (!dont_show) {
        //    show_image(im, "predictions");
        //}
...
void run_detector(int argc, char **argv)
{
    ...
    //Ryolo
    else if (0 == strcmp(argv[2], "test_Ryolo")) test_Rdetector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    ...
    //Ryolo
    else if (0 == strcmp(argv[2], "train_Ryolo")) train_Rdetector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, mjpeg_port, show_imgs, benchmark_layers, chart_path);
